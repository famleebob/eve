...
do_tar() {
  ...
  #* add code to "fix up" the root file system image
  #*  answers a distinct problem with the archive
  #*  generated by `linuxkit`
  tmp_workdir=`mktemp -d`
  cd ${tmp_workdir}
  alt_tarfile=${tarfile}.$$
  tar xf ${tarfile} --exclude "dev/*" || :
  tarfilter ${tarfile} ${alt_tarfile} var/lock etc/ssl/certs
  mv var/lock run
  ln -s /run/lock var/lock
  mkdir -p var/lib/ca-certificates
  mv etc/ssl/certs var/lib/ca-certificates/pem

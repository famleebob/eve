...
func verifySignature(log *base.LogObject, certByte []byte, interm *x509.CertPool) error {
	...
	if now.After(lastCertTimeCheck.AddDate(0, 0, 1)) {
		...
		// check the cert validity using cert built-in Verify method
		// for now just log any threshold violations, and set the status
		if warnT.After(notAfter) {
			...
			// if the status should be allCertsAreValid, set it
			// assumes failed once before, passed now, so reset
	...
	log.Tracef("Verify Cert after,  now %v, last %v",
		now, lastCertTimeCheck)

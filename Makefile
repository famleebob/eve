...
PKGS_$(ZARCH)=pkg/measure-config pkg/debug pkg/dnsmasq pkg/dom0-ztools \
              ...
              pkg/rngd pkg/uefi pkg/wlan pkg/xen pkg/xen-tools pkg/wlan \
              pkg/wwan pkg/guacd pkg/vtpm pkg/watchdog
...
$(BUILDTOOLS_BIN)/tarfilter: $(BUILDTOOLS_BIN)/../src/tarfilter.go
	GO111MODULE=on CGO_ENABLED=0 go build -o $@ $<

build-tools: $(LINUXKIT) $(BUILDTOOLS_BIN)/tarfilter
	@echo Done building $<
